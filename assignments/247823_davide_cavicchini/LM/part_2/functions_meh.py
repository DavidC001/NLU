...
class NT_AvSGD(optim.ASGD):
    def __init__(self, model, lr=1, n=5):
    ...
    for epoch in pbar:
        ...
        if epoch % 1 == 0:
            ...
            if 't0' in optimizer.param_groups[0]:
                    # L is the number of cycles in a epoch
                    if optimizer.T==0:
                        if optimizer.t>optimizer.n and ppl_dev > min(optimizer.logs[:optimizer.t-optimizer.n]):
                            optimizer.T = optimizer.k
                            # set t0 of ASGD to 0
                            for group in optimizer.param_groups:
                                group['t0'] = 0
                            print("averaging started, at iteration", optimizer.k, " after ", optimizer.t, " cycles")
